...
sf::st_write(obj = designated_climate_zones, delete_dsn = TRUE, dsn = "./data/weather-stations/dws/wheat/ready/designated_climate_zones_wheat_egypt_only_v2.gpkg")
...
# New weather station
ws_extra <- data.frame(station_name = c("Luxor"), station_id = paste0(0000, 1:1), latitude = 25.683333, longitude = 32.65, country_name = c('Egypt'), climatezone_id = 81808002)
ws <- rbind(ws_extra, ws)
...
sf::st_write(obj = designated_weather_stations, delete_dsn = TRUE, dsn = "./data/weather-stations/dws/wheat/ready/designated_weather_stations_wheat_egypt_only_v2.gpkg")
...
designated_buffer_zones <- 
  ...
  split(rownames(.)) %>%
  map(function(climate_zone){
    climate_zone$bz_nha <- as.numeric(raster::extract(crop_mask, climate_zone, fun = sum, na.rm = TRUE))
    return(climate_zone)
  }) %>%
  reduce(rbind) 
...
# Export buffer zones
sf::st_write(obj = designated_buffer_zones, delete_dsn = TRUE, dsn = "./data/weather-stations/dws/wheat/ready/designated_buffer_zones_wheat_egypt_only_v2.gpkg")
...
l_pols[["country"]] %>%
  ...
  st_drop_geometry() %>%
  arrange(GID_0) %>%
  readr::write_csv(path = here(path_water_regime, "SPAM_wheat_water_regime_COUNTRY_v2.csv"))
l_pols[["dcz"]] %>%
  dplyr::select(NAME_0, GID_0, GYGA_CZ, ID, irrigated, rainfed, per_irrigated, per_rainfed) %>%
  st_drop_geometry() %>%
  arrange(GID_0, GYGA_CZ) %>%
  readr::write_csv(path = here(path_water_regime, "SPAM_wheat_water_regime_CZ_v2.csv"))
l_pols[["dbz"]] %>%
  dplyr::select(NAME_0, GID_0, GYGA_CZ, station_id, station_name, irrigated, rainfed, per_irrigated, per_rainfed) %>%
  st_drop_geometry() %>%
  arrange(GID_0, ) %>%
  readr::write_csv(path = here(path_water_regime, "SPAM_wheat_water_regime_RWS_v2.csv"))
