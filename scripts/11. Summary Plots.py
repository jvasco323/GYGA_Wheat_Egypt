...
ax1.set_title('')
ax1.set_xticks(ind)
ax1.set_yticklabels([0, 2, 4, 6, 8, 10, 12], family='sans-serif', fontsize=12.5, color='black')
...
ind = np.arange(N)  # the x locations for the groups
width = 0.75        # the width of the bars
Ya = list([100*6.8/9.7, 100*6.2/8.4])
Yw = list([100, 100])
...
for site in sites:
    ...
    # ------------------------------------------------------------------------------------------------------------------
    plt.savefig(os.path.join(output_dir, r"./daily_wheat_{site}.pdf".format(site=site)), bbox_inches='tight')
